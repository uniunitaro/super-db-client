...
- 以下の方針でコメントを書いてください、あなたはコメントを書かない傾向にあるので、コメントを書くように意識してください
  - コードを読めば分かること（処理内容の逐語説明）はコメントしない
  - コメントは「コードだけでは伝わらない意図」を書く（採用理由、設計判断、既知バグ/仕様回避、パフォーマンス・互換性・安全性の都合、将来の拡張前提など）
  - 別セッションのAI/人が初見で読んでも理解が速くなる情報がある場合は、短く補足する（前提・制約・注意点・罠）
